...
static void cwav_updateVolumeImpl(cwav_t* cwav, float volume, float pan, int leftChannel, int rightChannel, u8 multipleID)
{
	...
	if (leftChannel >= 0 && leftChannel < (int)cwav->channelcount)
	{
		...
		{
			cwavEnvUpdateVolume(cwav->playingChanIds[multipleID][leftChannel], volume, pan);
		}
	}
	if (rightChannel >= 0 && rightChannel < (int)cwav->channelcount)
	{
		...
		{
			cwavEnvUpdateVolume(cwav->playingChanIds[multipleID][rightChannel], volume, pan);
		}
	}
	if (leftChannel < 0 && rightChannel < 0)
	{
		for (u32 i = 0; i < cwav->channelcount; i++)
		{
			...
			{
				cwavEnvUpdateVolume(cwav->playingChanIds[multipleID][i], volume, pan);
			}
		}
	}
}
...
void cwavUpdateVolume(CWAV* cwav, int leftChannel, int rightChannel)
{
	...
	for (int i = 0; i < cwav_->totalMultiplePlay; i++)
	{
		cwav_updateVolumeImpl(cwav_, cwav->volume, cwav->monoPan, leftChannel, rightChannel, i);
	}
}
